...
<section class="section dashboard">
    <div class="row">
        {{-- supervisor approve --}}
        @if(!in_array(Auth::user()->position->position_code, [
            'ITD1',
            'MGT2',
            'HRGA1',
            'MGT5',
                    ...
                    <div class="table-responsive  mt-2 overflow-x: auto;">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th style="text-align: center;">Form Permit</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php
                            ...
                            @forelse ($cutiBawahan  as $items)
                                <tr>
                                    ...
                                    <td>
                                        @if ($items->approve1_name !== null)
                                        <a href="{{ route('permit.details', $items->id)}}" class="btn btn-success">Check Form</a>
                                        @else
                                        <a href="{{ route('permit.form', $items->id)}}" class="btn btn-primary">View</a>
                                        @endif
                                    </td>
                                </tr>
                                @php
                        ...
                        <tbody>
                            ...
                            @forelse ($approve2 as $data)
                                <tr>
                                    ...
                                    <td>
                                        @if ($data->approve2_name !== null)
                                            <a href="{{ route('permit.details', $data->id)}}" class="btn btn-success">Check Form</a>
                                        @else
                                            <a href="{{ route('permit.form', $data->id)}}" class="btn btn-primary">View</a>
                                        @endif
                                    </td>
                                </tr>
                                @php
        ...
        <!-- MGT5 Approve-->
        @if (Auth::user()->level == 'Developer' || Auth::user()->position->position_code == 'MGT5')
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col">
                            <h5 class="card-title ">Request Unpaid Leave</h5>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive  mt-2 overflow-x: auto;">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th>Department</th>
                                <th>Position</th>
                                <th>Number Leave Days</th>
                                <th>Start Date</th>
                                <th>End Date</th>
                                <th>Back Office Date</th>
                                <th>Destination</th>
                                <th>Phone Number</th>
                                <th>Approve Spv</th>
                                <th>Approve Manager</th>
                                <th>Date Approved</th>
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php
                                $iteration = 1;
                            @endphp
                            @forelse ($approveunpaid  as $unpaid)
                                @if ($unpaid->approve2_name === null && $unpaid->approve1_name === null)
                                @else
                                    <tr>
                                        <td>{{ $iteration }}</td>
                                        <td>{{ $unpaid->user->employee_id }}</td>
                                        <td>{{ $unpaid->user->name }}</td>
                                        <td>{{ $unpaid->department }}</td>
                                        <td>{{ $unpaid->position_name }}</td>
                                        <td>{{ $unpaid->jumlah_hari}}</td>
                                        <td>{{ $unpaid->from_date }}</td>
                                        <td>{{ $unpaid->to_date }}</td>
                                        <td>{{ $unpaid->backoffice_date }}</td>
                                        <td>{{ $unpaid->tujuan }}</td>
                                        <td>{{ $unpaid->telp }}</td>
                                        <td>{{ $unpaid->approve1_name ?? '-'}}</td>
                                        <td>{{ $unpaid->approve2_name ?? '-'}}</td>
                                        <td>{{ $unpaid->approval3_date ? date('d-m-Y', strtotime($unpaid->approval3_date)) : '' }}</td>
                                        <td>
                                            @if ($unpaid->approval3_date !== null)
                                                <a href="{{ route('permit.details', $unpaid->id)}}" class="btn btn-success">Check Form</a>
                                            @else
                                                <a href="{{ route('permit.unpaid', $unpaid->id)}}" class="btn btn-primary">View</a>
                                            @endif
                                        </td>
                                    </tr>
                                    @php
