...
<style>
    ...
    .card-body {
    overflow-x: auto; /* Tampilkan bilah geser horizontal jika tabel terlalu lebar */
    }
    .text {
        display: none;
    }
...
<div class="pagetitle">
    <h1>Medical Leave Approval</h1>
    <nav>
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="/dashboard">Home</a></li>
        <li class="breadcrumb-item">Approval List</li>
        <li class="breadcrumb-item active">Medical Leave</li>
      </ol>
    </nav>
</div>
                        ...
                        <tbody>
                            ...
                            @forelse ($cutiBawahan  as $items)
                                <tr>
                                    ...
                                    <td>
                                        @if($items->pdf_file)
                                            <a href="{{ Storage::url('public/pdf/Medical Leave/' . $items->user->nik . '-' . str_replace(' ', ' ', $items->user->name) . '/' . $items->pdf_file) }}" target="_blank" class="btn btn-danger"><i class="fa-regular fa-file-pdf"></i></a>
                                        @else
                                            No PDF
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('medicalleave.details', $items->id)}}" class="btn btn-success">Check Form</a>
                                        @else
                                        <a href="{{ route('medicalleave.form', $items->id)}}" class="btn btn-primary">View</a>
                                        @endif
                                    </td>
                                </tr>
                                @php
                        ...
                        <tbody>
                            ...
                            @forelse ($approve2 as $data)
                                <tr>
                                    ...
                                    <td>
                                        @if($data->pdf_file)
                                            <a href="{{ Storage::url('public/pdf/Medical Leave/' . $data->user->nik . '-' . str_replace(' ', ' ', $data->user->name) . '/' . $data->pdf_file) }}" target="_blank" class="btn btn-danger"><i class="fa-regular fa-file-pdf"></i></a>
                                        @else
                                            No PDF
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('medicalleave.details', $data->id)}}" class="btn btn-success">Check Form</a>
                                        @else
                                        <a href="{{ route('medicalleave.form', $data->id)}}" class="btn btn-primary">View</a>
                                        @endif
                                    </td>
                                </tr>
                                @php
